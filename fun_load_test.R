...
#         site.name="MPI"; dir.plot="./png_plot/"
#	 dir.name="/lfs/home/ychen/lfs_dir/EC_DATA/"
#         dir.subname="2021"; yyyy="2021";mm="05";dd="01";hhmm="0030";ld.plot="TRUE";ld.spec="TRUE";ld.na=TRUE;ld.spik=TRUE
...
#	file.name <- paste(dir.name,"/","CSSAGRI_",yyyy,"_",mm,"_",dd,"_",hhmm,".dat",sep="")
       	file.name <- paste(dir.name,"/",site.name,"/30min/",dir.subname,"/",site.name,"_",yyyy,"_",mm,"_",dd,"_",hhmm,".txt",sep="")
	  ...
	  # }
         ...
         # Gapfill the data 
            ...
            # if ( wrk.time.stamp > ch4.date.stamp ) { 
             ...
             load.data$Uzc  <- fun_despike(x=as.numeric(load.data$W..m.s.),  nstd=c(3.5))
             load.data$Uxc  <- fun_despike(x=as.numeric(load.data$U..m.s.),  nstd=c(3.5))
             load.data$Uyc  <- fun_despike(x=as.numeric(load.data$V..m.s.),  nstd=c(3.5))
             load.data$H2O  <- fun_despike(x=as.numeric(load.data$H2O..g.m.3.),  nstd=c(3.5))
             load.data$CO2  <- fun_despike(x=as.numeric(load.data$CO2..mg.m.3.),  nstd=c(3.5))
             load.data$Tsc  <- fun_despike(x=as.numeric(load.data$T..C.),  nstd=c(3.5))
             load.data$CH4  <- fun_despike(x=as.numeric(load.data$CH4..mmol.m.3.),  nstd=c(3.5)) 
             load.data$cell_press  <- fun_despike(x=as.numeric(load.data$Pressure..kPa.),  nstd=c(3.5)) 
             # 
         ...
         if (ld.na) {
          ...
          #  if ( wrk.time.stamp > ch4.date.stamp ) { 
            ...
            #load.data$CH4 <- fun_na_fill(load.data$CH4,  plot.ld=F, plot.name=c(plot.name))
           # }else{
	...
	if (ld.plot) {
        ...
        plot(na.omit(load.data$CH4),typ="l",col="red", panel.first = grid())
